...
data = []
try:
    with open('reinference.json', 'r', encoding='utf-8') as file:
        data = json.load(file)
...
def process_prompt(index, prompt, lock, f):
    ...
    matches_duration = re.findall(signal_answer_pattern_duration,  prompt["output"]["content"])
    print(matches_duration)
    if  '2' not in matches_duration:
        output = llama3170B([messages])

        piece = {'query': prompt["instruction"], 'responses':[output, prompt["output"]["content"]], "scores": [0.8, 0.2]}
...
def custom_Llama_inference(model, tokenizer, sampling_params, syn_prompts):
    ...
    with open('score_based_data.json', 'w', encoding='utf-8') as f:
        f.write("[\n")
