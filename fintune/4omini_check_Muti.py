...
data = []
try:
    with open('jinan2_Llama_log_220.json_formatted.json', 'r', encoding='utf-8') as file:
        data = json.load(file)
...
def prepare_messages(prompt):
    ...
    messages = [
        ...
        {
            "role": "user",
            "content":"YOU SHOULD JUDGE THE QUERY AND ANSWER IF THE ANSWER IS CORRECT AND REASONABLE. YOU SHOULD ANSWER <Judgement>Approved</Judgement> or <Judgement>Rejected</Judgement>\n"
            f"Query: {prompt['instruction']}\n Answer: {prompt['output']}",
                }
    ]
    return messages
...
def process_prompt(index, prompt, lock, f):
    ...
    signal_answer_pattern_phase = r'<Judgement>(.*?)</Judgement>'
    matches_phase = re.findall(signal_answer_pattern_phase, output)
    result =  matches_phase[-1]
    print(result)
...
def custom_Llama_inference(model, tokenizer, sampling_params, syn_prompts):
    ...
    with open('reinference.json', 'w', encoding='utf-8') as f:
        f.write("[\n")
